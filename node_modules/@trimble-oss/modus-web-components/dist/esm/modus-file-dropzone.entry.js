...
import { a as IconCancel, I as IconUploadCloud } from './icon-upload-cloud-4225b13f.js';
...
const ModusFileDropzone = class {
  ...
  render() {
    return (h(Host, { "aria-label": this.ariaLabel, role: "button" }, h("div", { class: "modus-file-dropzone" }, h("input", { onChange: this.onFileChange, multiple: this.multiple, ref: (el) => (this.fileInput = el), type: "file" }), h("div", { class: "header" }, h("label", null, this.label), h("span", null, this.description)), h("div", { class: `dropzone ${this.fileDraggedOver ? 'highlight' : null} ${this.error ? 'error' : null}`, onDragLeave: (e) => this.onDragLeave(e), onDragOver: (e) => this.onDragOver(e), onDrop: (e) => this.onDrop(e), style: { height: this.dropzoneHeight, width: this.dropzoneWidth }, tabIndex: 0 }, this.includeStateIcon && (this.error ? h(IconCancel, { size: '36' }) : h(IconUploadCloud, { size: '36' })), !this.error && (h("div", null, "Drag files here", !this.fileDraggedOver && (h("span", null, ' ', "or", h("span", { class: "browse", onClick: this.openBrowse }, ' ', "browse"), ' ', "to upload")), ".")), this.error && (h("div", { class: "error-messages" }, this.errorMessageTop && h("span", null, this.errorMessageTop), this.errorMessageBottom && h("span", null, this.errorMessageBottom)))))));
  }
};
