...
const ModusRadioButton = (props) => (index.h("div", { class: `modus-radio-button ${props.disabled ? 'disabled' : ''}`, onClick: () => props.handleButtonClick(props.id), onKeyDown: (event) => props.handleKeydown(event, props.id), tabIndex: 0 },
  index.h("div", { class: "radio" },
    index.h("input", { checked: props.checked, disabled: props.disabled, name: props.name, type: "radio" }),
    index.h("span", { class: "checkmark" }),
...
const ModusRadioGroup = class {
  ...
  render() {
    return (index.h("div", { class: "modus-radio-group", "aria-label": this.ariaLabel }, this.radioButtons.map((radioButton) => {
      return (index.h(ModusRadioButton, { checked: radioButton.checked, disabled: radioButton.disabled, label: radioButton.label, name: this.name, id: radioButton.id, handleButtonClick: (id) => this.handleButtonClick(id), handleKeydown: (event, id) => this.handleButtonKeydown(event, id) }));
    })));
  }
...
exports.modus_radio_group = ModusRadioGroup;
