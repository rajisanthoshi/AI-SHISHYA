...
function CellFormatter(Comp, props) {
  return !Comp ? null : isFunction(Comp) ? index.h(Comp, Object.assign({}, props)) : Comp;
}
/**
...
const ModusTableCellLinkElement = ({ link, onLinkClick }) => {
  return (index.h("div", { class: "cell-link wrap-text", onClick: () => onLinkClick(link) }, link.display));
};
...
const ModusTableCellContent = ({ cell, onLinkClick }) => {
  ...
  return (index.h("span", { class: "wrap-text" }, cellValue ? (
  /** Link */
  getCellType(cellValue['_type'], cell.column.columnDef[modusTable_constants.COLUMN_DEF_DATATYPE_KEY]) === modusTable_constants.COLUMN_DEF_DATATYPE_LINK ? (index.h(ModusTableCellLinkElement, { link: cellValue, onLinkClick: (link) => {
      onLinkClick(link);
    } })) : (
  /** Default*/
...
const ModusTableCellExpandIcons = ({ cellIndex, row }) => {
  return (index.h("span", { class: "expand", style: { paddingLeft: `${cellIndex === 0 ? row.depth * 2 : 0}rem` }, onClick: row.getToggleExpandedHandler() }, cellIndex === 0 && row.getCanExpand() ? (row.getIsExpanded() ? (index.h(iconChevronUpThick.IconChevronUpThick, { size: '24' })) : (index.h(iconChevronDownThick.IconChevronDownThick, { size: '24' }))) : ('')));
};

const ModusTableCell = ({ cell, row, cellIndex, rowsExpandable, frozenColumns, onLinkClick, }) => {
  const { id, column } = cell;
  return (index.h("td", { key: id, class: `
          ${frozenColumns.includes(column.id) ? 'sticky-left' : ''}
          ${column.getIsResizing() ? 'active-resize' : ''}
      `, style: {
      width: `${column.getSize()}px`,
    } },
    index.h("div", { class: `table-cell wrap-text${column.columnDef[modusTable_constants.COLUMN_DEF_DATATYPE_KEY] === modusTable_constants.COLUMN_DEF_DATATYPE_INTEGER ? 'text-align-right' : ''}` },
      /** Expand or collaps icon */
      rowsExpandable ? index.h(ModusTableCellExpandIcons, { cellIndex: cellIndex, row: row }) : '',
      /** Cell content */
      index.h(ModusTableCellContent, { cell: cell, onLinkClick: (link) => {
          onLinkClick(link);
        } }))));
};
...
function columnResizingOnKeyDown(event, table, column) {
  ...
  if (eventKey === modusTable_constants.KEYBOARD_ENTER ||
    eventKey === modusTable_constants.KEYBOARD_SPACE ||
    eventKey === modusTable_constants.KEYBOARD_TAB ||
    eventKey === modusTable_constants.KEYBOARD_ESCAPE) {
    ...
    if (eventKey !== modusTable_constants.KEYBOARD_TAB && eventKey !== modusTable_constants.KEYBOARD_ESCAPE) {
  ...
  else if (eventKey === modusTable_constants.KEYBOARD_LEFT || eventKey === modusTable_constants.KEYBOARD_RIGHT) {
    ...
    columnInfo[column.id] = columnSize + (eventKey === modusTable_constants.KEYBOARD_LEFT ? -5 : 5);
...
const ModusTableColumnResizingHandler = ({ table, header, onMouseEnter, onMouseLeave, }) => {
  return (index.h("div", { tabindex: 0, onMouseDown: header.getResizeHandler(), onTouchStart: header.getResizeHandler(), onKeyDown: throttle(table, header.column), onMouseEnter: () => onMouseEnter(), onMouseLeave: () => onMouseLeave(), class: "resize-handle" }));
};
...
function showSortingStatus(column, isColumnResizing) {
  return isColumnResizing
    ...
    : column.getIsSorted() === 'asc'
      ? modusTable_constants.SORTED_ASCENDING
      : column.getIsSorted() === 'desc'
        ? modusTable_constants.SORTED_DESCENDING
        : column.getNextSortingOrder() === 'asc'
          ? modusTable_constants.SORT_ASCENDING
          : modusTable_constants.SORT_DESCENDING;
}
/**
...
function sortOnKeyDown(column, event) {
  if (event.key.toLowerCase() === modusTable_constants.KEYBOARD_ENTER) {
    column.toggleSorting();
    event.preventDefault();
  }
  event.stopPropagation();
}
const ModusTableHeaderSort = ({ column, showSortIconOnHover, isColumnResizing, }) => {
  return (index.h("modus-tooltip", { text: showSortingStatus(column, isColumnResizing), position: "bottom" }, index.h("span", { tabindex: "0", "aria-label": showSortingStatus(column, isColumnResizing), role: "button", onClick: column.getToggleSortingHandler(), onKeyDown: (event) => sortOnKeyDown(column, event), onMouseDown: (event) => event.stopPropagation(), class: "sort-icon-container" },
    index.h("span", { class: `sort-icon
              ${!column.getIsSorted() && 'disabled'}
              ${showSortIconOnHover ? 'hidden' : ''}
            ` }, column.getIsSorted() === 'asc' ? index.h(iconChevronUpThick.IconSortAZ, { size: '16' }) : index.h(iconChevronUpThick.IconSortZA, { size: '16' })))));
};

/**
 * Modus Table Header
 */
const ModusTableHeader = ({ table, header, isNestedParentHeader, showSortIconOnHover, columnReorder, isColumnResizing, frozenColumns, onDragStart, onMouseEnterResize, onMouseLeaveResize, }) => {
  let elementRef;
  const { column, id, colSpan, isPlaceholder, getSize } = header;
  return (index.h("th", { tabindex: `${!isColumnResizing && columnReorder ? '0' : ''}`, key: id, colSpan: colSpan, 
    /**
    ...
    index.h("div", { class: column.getCanSort() && 'can-sort' },
      index.h("span", null, column.columnDef.header),
      column.getCanSort() && (index.h(ModusTableHeaderSort, { column: column, showSortIconOnHover: showSortIconOnHover, isColumnResizing: isColumnResizing })))),
    column.getCanResize() ? (index.h(ModusTableColumnResizingHandler, { table: table, header: header, onMouseEnter: () => onMouseEnterResize(), onMouseLeave: () => onMouseLeaveResize() })) : null));
};

const IconArrowDown = (props) => {
  var _a, _b, _c;
  return (index.h("svg", { class: "icon-arrow-down", xmlns: "http://www.w3.org/2000/svg", fill: (_a = props.color) !== null && _a !== void 0 ? _a : 'currentColor', height: (_b = props.size) !== null && _b !== void 0 ? _b : 16, width: (_c = props.size) !== null && _c !== void 0 ? _c : 16, onClick: props.onClick, "data-test-id": "iconArrowDown", viewBox: "0 0 24 24" },
    index.h("path", { d: "M15 12h3.79c.45 0 .67.54.35.85l-6.44 6.44a.996.996 0 0 1-1.41 0l-6.44-6.44A.5.5 0 0 1 5.2 12h3.79V5c0-.55.45-1 1-1h4c.55 0 1 .45 1 1v7Z" })));
};

const IconArrowUp = (props) => {
  var _a, _b, _c;
  return (index.h("svg", { class: "icon-arrow-up", xmlns: "http://www.w3.org/2000/svg", fill: (_a = props.color) !== null && _a !== void 0 ? _a : 'currentColor', height: (_b = props.size) !== null && _b !== void 0 ? _b : 16, width: (_c = props.size) !== null && _c !== void 0 ? _c : 16, onClick: props.onClick, "data-test-id": "iconArrowUp", viewBox: "0 0 24 24" },
    index.h("path", { d: "M14 20h-4c-.55 0-1-.45-1-1v-7H5.21c-.45 0-.67-.54-.35-.85l6.44-6.44a.996.996 0 0 1 1.41 0l6.44 6.44a.5.5 0 0 1-.35.85h-3.79v7c0 .55-.45 1-1 1Z" }),
    ' '));
};
...
const ModusTableColumnDragItem = ({ draggingState }) => {
  ...
  return index.h("div", { style: Object.assign({}, dragContainerStyle), ref: (el) => el && el.appendChild(dragContent), class: "drag-content" });
};
const ModusTableColumnDropIndicator = ({ position }) => {
  ...
  return (index.h("div", { style: Object.assign({}, dragContainerStyle), class: "drop-allow-arrows" },
    index.h(IconArrowDown, { size: '16' }),
    index.h(IconArrowUp, { size: '16' })));
};

const ModusTablePagination = ({ table, totalCount, pageSizeList, }) => {
  ...
  return (index.h("div", { class: "pagination-container" },
    index.h("div", { class: "items-per-page" },
      index.h("span", null, modusTable_constants.PAGINATION_PAGEVIEW_TEXT),
      index.h("modus-select", { "options-display-prop": "display", options: optionsList, onValueChange: handleChange, value: selectedPageSize })),
    index.h("div", { class: "pagination-and-count" },
      index.h("div", { class: "total-count" },
        index.h("span", null, modusTable_constants.PAGINATION_SUMMARY_TEXT),
        index.h("span", null, pageIndex * pageSize + 1),
        index.h("span", null, "-"),
        index.h("span", null, pageIndex + 1 === getPageCount()
          ? options.paginateExpandedRows
            ? getExpandedRowModel().rows.length
            : totalCount
          : (pageIndex + 1) * pageSize),
        index.h("span", null, "of"),
        index.h("span", null, options.paginateExpandedRows ? getExpandedRowModel().rows.length : totalCount),
        ' '),
      index.h("modus-pagination", { "active-page": 1, "max-page": getPageCount(), "min-page": 1, onPageChange: (event) => setPageIndex(event.detail - 1) }))));
};
...
const ModusTableSummaryRow = ({ footerGroups, tableData, frozenColumns, rowSelection, }) => {
  return (index.h("tfoot", null, footerGroups.map((group) => (index.h("tr", { class: "summary-row" },
    rowSelection && (index.h("td", { id: modusTable_constants.COLUMN_DEF_ROW_SELECTION_ID, key: modusTable_constants.COLUMN_DEF_ROW_SELECTION_ID, class: modusTable_constants.COLUMN_DEF_ROW_SELECTION_CSS })),
    group.headers.map((header) => (index.h("td", { id: header.id, key: header.id, class: `
                ${frozenColumns.includes(header.id) ? 'sticky-left' : ''}
                ${header.column.columnDef[modusTable_constants.COLUMN_DEF_DATATYPE_KEY] === modusTable_constants.COLUMN_DEF_DATATYPE_INTEGER ? 'text-align-right' : ''}
              ` }, header.column.columnDef[modusTable_constants.COLUMN_DEF_SHOWTOTAL]
...
class TableHeaderDragDrop {
  ...
  setValues(columnOrder, columnReorder, columnResizeEnabled, tableHeaderRowRef, table, itemDragState, frozenColumns) {
  ...
  handleKeyDown(event) {
    ...
    // Move right or left
    if (event.key.toLowerCase() === modusTable_constants.KEYBOARD_RIGHT || event.key.toLowerCase() === modusTable_constants.KEYBOARD_LEFT) {
      clientX = event.key.toLowerCase() === modusTable_constants.KEYBOARD_RIGHT ? clientX + 5 : clientX - 5;
      this.updateNodeAndDragState(clientX, clientY);
      event.preventDefault();
      // Drop Item
    }
    else if (event.key.toLowerCase() === modusTable_constants.KEYBOARD_ENTER && ((_b = this.itemDragState) === null || _b === void 0 ? void 0 : _b.dropColumnId)) {
      this.handleDrop();
      return;
      // Exit on Escape Or Tab
    }
    else if (event.key.toLowerCase() === modusTable_constants.KEYBOARD_ESCAPE || event.key.toLowerCase() === modusTable_constants.KEYBOARD_TAB) {
      this.itemDragState = null;
      return;
    }
  }
  ...
  updateNodeAndDragState(clientX, clientY) {
    ...
    if ((node === null || node === void 0 ? void 0 : node.id) && node.id !== newDragState.draggedColumnId) {
      newDragState = Object.assign(Object.assign({}, newDragState), { dropColumnId: node.id, dropIndicator: node.getBoundingClientRect() });
    }
    else {
...
const ModusTable = class {
  constructor(hostRef) {
    ...
    this.pageSizeList = modusTable_constants.PAGINATION_DEFAULT_SIZES;
  ...
  renderToolBar() {
    return (this.toolbar &&
      this.toolbarOptions && (index.h("modus-table-toolbar", { table: this.table, options: this.toolbarOptions }, index.h("div", { slot: "group-left" }, index.h("slot", { name: "groupLeft" })), index.h("div", { slot: "group-right" }, index.h("slot", { name: "groupRight" })))));
  }
  renderMain() {
    ...
    return (index.h(index.Fragment, null, index.h("div", { class: tableContainerClass, style: { maxHeight: this.maxHeight } }, this.renderTable(), index.h("modus-table-filler-column", { "summary-row": this.summaryRow, "cell-borderless": cellBorderless, ref: (el) => (this.fillerColumnRef = el) })), index.h("slot", { name: "customFooter" })));
  }
  renderTable() {
    var _a, _b;
    ...
    const tableStyle = this.fullWidth
      ...
      : totalSize > 0
        ...
        : { tableLayout: 'fixed' };
    return (index.h("table", { "data-test-id": "main-table", class: tableMainClass, style: tableStyle, ref: (el) => (this.tableRef = el) }, this.renderTableHeader(multiple), this.renderTableBody(multiple), this.renderTableFooter()));
  }
  renderTableBody(multipleRowSelection) {
    var _a;
    return (index.h("tbody", null, (_a = this.table.getRowModel()) === null || _a === void 0 ? void 0 : _a.rows.map((row) => {
      var _a, _b;
      const isChecked = row.getIsSelected() && (((_a = row.subRows) === null || _a === void 0 ? void 0 : _a.length) ? row.getIsAllSubRowsSelected() : true);
      return (index.h("tr", { key: row.id, class: { 'enable-hover': this.hover, 'row-selected': isChecked } }, this.rowSelection && (index.h("td", { class: "row-checkbox sticky-left" }, index.h("modus-checkbox", { checked: isChecked, indeterminate: multipleRowSelection && row.getIsSomeSelected(), onCheckboxClick: () => row.toggleSelected() }))), (_b = row.getVisibleCells()) === null || _b === void 0 ? void 0 :
        _b.map((cell, cellIndex) => {
          return (index.h(ModusTableCell, { cell: cell, row: row, cellIndex: cellIndex, rowsExpandable: this.rowsExpandable, frozenColumns: this.frozenColumns, onLinkClick: (link) => this.cellLinkClick.emit(link) }));
        })));
    })));
  }
  renderTableHeader(multipleRowSelection) {
    ...
    return (index.h("thead", { class: tableHeadClass }, headerGroups === null || headerGroups === void 0 ? void 0 : headerGroups.map((headerGroup, index$1) => {
      var _a;
      return (index.h("tr", { key: headerGroup.id, ref: (element) => (this.tableHeaderRowRef = element) }, this.rowSelection && (index.h("th", { class: "row-checkbox sticky-left" }, multipleRowSelection && (index.h("modus-checkbox", { checked: this.table.getIsAllRowsSelected(), indeterminate: this.table.getIsSomeRowsSelected(), onCheckboxClick: this.table.getToggleAllRowsSelectedHandler() })))), (_a = headerGroup.headers) === null || _a === void 0 ? void 0 :
        _a.map((header) => {
          return (index.h(ModusTableHeader, { table: this.table, header: header, isNestedParentHeader: index$1 < headerGroups.length - 1, showSortIconOnHover: this.showSortIconOnHover, columnReorder: this.columnReorder, isColumnResizing: this.isColumnResizing, frozenColumns: this.frozenColumns, onDragStart: (event, id, elementRef, mouseInteracted) => this.handleDragStart(event, id, elementRef, mouseInteracted), onMouseEnterResize: () => (this.isColumnResizing = true), onMouseLeaveResize: () => (this.isColumnResizing = false) }));
        })));
    })));
  }
  renderTableFooter() {
    const footerGroups = this.table.getFooterGroups();
    return this.summaryRow ? (index.h(ModusTableSummaryRow, { footerGroups: [footerGroups[0]], tableData: this.data, frozenColumns: this.frozenColumns, rowSelection: this.rowSelection })) : null;
  }
  renderPagination() {
    return (this.pagination && (index.h(ModusTablePagination, { table: this.table, totalCount: this.data.length, pageSizeList: this.pageSizeList })));
  }
  render() {
    var _a;
    return (index.h(index.Host, null, index.h("div", { style: { maxWidth: this.maxWidth } }, this.renderToolBar(), this.renderMain(), this.renderPagination(), index.h(ModusTableColumnDragItem, { draggingState: this.itemDragState }), index.h(ModusTableColumnDropIndicator, { position: (_a = this.itemDragState) === null || _a === void 0 ? void 0 : _a.dropIndicator }))));
  }
  static get watchers() { return {
    "columns": ["onChangeOfColumns"],
    "data": ["onChangeOfData"],
    "columnResize": ["updateColumnResize"],
    "sort": ["updateSort"],
    "toolbarOptions": ["onChangePanelOptions"],
    "rowSelectionOptions": ["onRowSelectionOptionsChange"],
    "columnReorder": ["updateColumnReorder"],
    "rowsExpandable": ["onChangeOfRowsExpandable"],
    "itemDragState": ["handleItemDragState"]
  }; }
};
ModusTable.style = modusTableCss;

exports.modus_table = ModusTable;
