...
const modusContentTree_constants = require('./modus-content-tree.constants-428fec37.js');
...
const ModusTreeViewItem = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.checkboxClick = index.createEvent(this, "checkboxClick", 7);
    this.itemClick = index.createEvent(this, "itemClick", 7);
    this.itemExpandToggle = index.createEvent(this, "itemExpandToggle", 7);
    this.itemAdded = index.createEvent(this, "itemAdded", 7);
    ...
    this.CustomSlot = (_a) => {
      ...
      return (index.h("div", Object.assign({}, props, { class: `${className || ''} ${display ? '' : 'd-none'}` }), index.h("slot", { name: name, onSlotchange: () => this.handleDefaultSlotChange() }), showDefault && defaultContent));
    };
  ...
  render() {
    ...
    const sizeClass = `${modusContentTree_constants.TREE_ITEM_SIZE_CLASS.get(size || 'standard')}`;
    ...
    return (index.h("li", Object.assign({}, ariaControls, { class: `tree-item-container${selectionIndicator ? ' selected-indicator' : ''}` }), index.h("div", { class: treeItemClass, onFocus: () => this.handleFocus(), onClick: (e) => this.handleItemClick(e), onKeyDown: (e) => this.handleKeyDownTreeItem(e), ref: (el) => this.handleRefItemContent(el), tabindex: tabIndex }, index.h(this.CustomSlot, { className: `icon-slot drag-icon${!this.draggableItem ? ' hidden' : ''}`, defaultContent: index.h(IconMap.IconMap, { icon: "drag" }), name: this.SLOT_DRAG_ICON, onMouseDown: (e) => this.handleDrag(e) }), index.h("div", { "aria-disabled": "true", style: { paddingLeft: `${(level - 1) * 0.5}rem` } }), index.h("div", { class: `icon-slot${!expandable ? ' hidden' : ''}`, onClick: (e) => this.handleExpandToggle(e), onKeyDown: (e) => this.handleDefaultKeyDown(e, () => this.handleExpandToggle(e)), tabindex: expandable ? tabIndex : -1 }, index.h(this.CustomSlot, { className: "inline-flex rotate-right", defaultContent: index.h(IconMap.IconMap, { icon: "chevron-down-thick", size: "24" }), display: !expanded, name: this.SLOT_EXPAND_ICON }), index.h(this.CustomSlot, { className: "inline-flex", defaultContent: index.h(IconMap.IconMap, { icon: "chevron-down-thick", size: "24" }), display: expanded, name: this.SLOT_COLLAPSE_ICON })), (checkboxSelection || multiCheckboxSelection) && (index.h("div", { class: "icon-slot" }, index.h("modus-checkbox", { checked: checked, disabled: isDisabled, indeterminate: indeterminate, onClick: (e) => this.handleCheckboxClick(e), onKeyDown: (e) => this.handleDefaultKeyDown(e, () => this.handleCheckboxClick(e)), ref: (el) => (this.refCheckbox = el), tabIndexValue: tabIndex }))), index.h(this.CustomSlot, { className: "icon-slot", name: this.SLOT_ITEM_ICON, display: this.slots.has(this.SLOT_ITEM_ICON) }), index.h("div", { role: "heading", "aria-level": level, class: "label-slot" }, index.h(this.CustomSlot, { role: "button", name: this.SLOT_LABEL, defaultContent: this.editable ? (index.h("modus-text-input", { size: size == 'large' ? 'large' : 'medium', autoFocusInput: true, tabindex: 0, ref: (el) => (this.refLabelInput = el), value: this.label, onClick: (e) => this.handleLabelInputClick(e), onBlur: () => this.handleLabelInputBlur(), onKeyDown: (e) => this.handleLabelInputKeyDown(e) })) : (this.label) }))), index.h("ul", { class: treeItemChildrenClass, role: "tree" }, index.h("slot", { onSlotchange: () => this.handleTreeSlotChange() }))));
  }
...
ModusTreeViewItem.style = modusTreeViewItemCss;

exports.modus_tree_view_item = ModusTreeViewItem;
