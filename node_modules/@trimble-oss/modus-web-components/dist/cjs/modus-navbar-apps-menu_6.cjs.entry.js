...
const ModusNavbarAppsMenu = class {
  ...
  render() {
    ...
    return (index.h("div", { class: `apps-menu ${direction}`, onClick: (event) => event.preventDefault() }, (_a = this.apps) === null || _a === void 0 ? void 0 : _a.map((app) => (index.h("div", { class: "app-div" }, app.showCategory ? index.h("div", { class: "category" }, app.category) : null, index.h("a", { class: "app", href: app.url, onClick: (event) => this.clickAppHandler(event, app) }, index.h("img", { src: app.logoUrl }), index.h("div", { class: "right" }, index.h("div", { class: "name" }, app.name), app.description ? index.h("div", { class: "description" }, app.description) : null)))))));
  }
};
...
const ModusNavbarButtonMenu = class {
  ...
  render() {
    ...
    return (index.h("div", { class: menuClasses }, index.h("slot", null)));
  }
};
...
const ModusNavbarMainMenu = class {
  ...
  render() {
    ...
    return (index.h("div", { class: "main-menu", style: positionStyle, onClick: (event) => event.preventDefault() }, index.h("slot", null)));
  }
};
...
const ModusNavbarNotificationsMenu = class {
  ...
  render() {
    const direction = this.reverse ? 'reverse' : '';
    return (index.h("div", { class: `notifications-menu ${direction}`, onClick: (event) => event.preventDefault() }, index.h("div", { class: "title" }, "Notifications"), index.h("slot", null)));
  }
};
...
const ModusNavbarProfileMenu = class {
  ...
  render() {
    ...
    return (index.h("div", { class: `profile-menu ${direction} ${variant}`, onClick: (event) => event.preventDefault() }, index.h("div", { class: "user" }, this.avatarUrl ? (index.h("img", { class: "avatar", src: this.avatarUrl, alt: "Avatar" })) : (index.h("span", { class: "initials" }, this.initials)), index.h("div", null, index.h("div", { class: "username" }, this.username), index.h("div", { class: "email" }, this.email))), ((_a = this.links) === null || _a === void 0 ? void 0 : _a.length) ? (index.h("div", { class: "links" }, this.links.map((link) => {
      return (index.h("div", { class: "link", onClick: () => this.linkClick.emit(link.id) }, link.display));
    }))) : null, index.h("div", { class: "sign-out", onClick: () => this.signOutClick.emit() }, index.h("div", null, "Sign out"))));
  }
};
...
const ModusNavbarSearchOverlay = class {
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "overlay-wrapper" }, index.h("div", { class: "search-box" }, index.h("modus-text-input", { placeholder: "Search", size: "large", onValueChange: (event) => this.search.emit(event.detail), "include-search-icon": true, ref: (el) => (this.modusTextInput = el) })), index.h("div", { class: "navbar-button", "data-test-id": "close-button" }, index.h("span", { class: "navbar-button-icon", tabIndex: 0, onKeyDown: (event) => event.code === 'Enter' && this.close.emit(), onClick: () => this.close.emit() }, index.h(iconClose.IconClose, { size: "24" }))))));
  }
