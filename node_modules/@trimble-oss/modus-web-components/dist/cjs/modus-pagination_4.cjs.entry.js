...
const ModusPagination = class {
  ...
  renderPreviousPageControl() {
    return (this.maxPage - this.minPage >= 7 && (index.h("li", { "aria-label": "Previous", class: `${this.activePage != this.minPage ? 'hoverable' : 'disabled'}`, onClick: () => this.handleChevronClick(PaginationDirection.Previous), onKeyDown: (event) => this.handleChevronKeydown(event, PaginationDirection.Previous), tabIndex: 0 }, this.prevPageButtonText ? (index.h("span", { "data-test-id": "prev-button-text" }, this.prevPageButtonText)) : (index.h(iconChevronLeftThick.IconChevronLeftThick, { size: this.chevronSizeBySize.get(this.size) })))));
  }
  renderNextPageControl() {
    return (this.maxPage - this.minPage >= 7 && (index.h("li", { "aria-label": "Next", class: `${this.activePage != this.maxPage ? 'hoverable' : 'disabled'}`, onClick: () => this.handleChevronClick(PaginationDirection.Next), onKeyDown: (event) => this.handleChevronKeydown(event, PaginationDirection.Next), tabIndex: 0 }, this.nextPageButtonText ? (index.h("span", { "data-test-id": "next-button-text" }, this.nextPageButtonText)) : (index.h(iconChevronRightThick.IconChevronRightThick, { size: this.chevronSizeBySize.get(this.size) })))));
  }
  renderPageNumbers() {
    return this.pages.map((page) => {
      const isCurrentPage = page === this.activePage;
      if (page === '...') {
        return (index.h("li", { class: `${!isNaN(+page) ? 'hoverable' : ''}`, tabIndex: -1 }, page));
      }
      return (index.h("li", { "aria-current": isCurrentPage ? 'page' : null, class: `${page === this.activePage ? 'active' : ''} ${!isNaN(+page) ? 'hoverable' : ''}`, onClick: () => this.handlePageClick(+page), onKeyDown: (event) => this.handlePageKeydown(event, +page), tabIndex: 0 }, page));
    });
  }
  render() {
    return (index.h("nav", { "aria-label": this.ariaLabel, class: `${this.classBySize.get(this.size)}` }, index.h("ul", null, this.renderPreviousPageControl(), this.renderPageNumbers(), this.renderNextPageControl())));
  }
...
const ModusSelect = class {
  ...
  renderSubText() {
    if (this.errorText) {
      return index.h("label", { class: "sub-text error" }, this.errorText);
    }
    else if (this.validText) {
      return index.h("label", { class: "sub-text valid" }, this.validText);
    }
    else if (this.helperText) {
      return index.h("label", { class: "sub-text helper" }, this.helperText);
    }
    return null;
  }
  renderLabel() {
    return this.label || this.required ? (index.h("div", { class: "label-container" }, this.label ? index.h("label", null, this.label) : null, this.required ? index.h("span", { class: "required" }, "*") : null)) : null;
  }
  renderOptions() {
    var _a;
    return (_a = this.options) === null || _a === void 0 ? void 0 : _a.map((option) => {
      ...
      return (index.h("option", { value: optionId, key: optionId, selected: option === this.internalValue }, option[this.optionsDisplayProp]));
    });
  }
  render() {
    var _a;
    const selectClass = `${this.classBySize.get(this.size)} ${this.errorText ? 'error' : this.validText ? 'valid' : this.disabled ? 'disabled' : ''}`;
    return (index.h("div", { class: this.disabled ? 'disabled' : undefined }, this.renderLabel(), index.h("span", { class: "input-container" }, index.h("select", { disabled: this.disabled, class: selectClass, "aria-label": this.ariaLabel, onChange: (event) => {
...
const ModusTableFillerColumn = class {
  ...
  render() {
    return (index.h("table", { class: { 'cell-borderless': this.cellBorderless, 'd-none': !this.showFillerTable }, ref: (el) => (this.fillerTableRef = el) }, index.h("thead", null, index.h("tr", null, index.h("th", null))), index.h("tbody", null, index.h("tr", null, index.h("td", null))), this.summaryRow && (index.h("tfoot", null, index.h("tr", { class: "summary-row" }, index.h("td", null))))));
  }
  static get watchers() { return {
...
const ModusTablePanel = class {
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "table-toolbar" }, index.h("div", { class: "section" }, index.h("slot", { name: "group-left" })), index.h("div", { class: "section" }, index.h("slot", { name: "group-right" }), index.h("modus-table-dropdown-menu", { table: this.table, options: this.options })))));
  }
};
ModusTablePanel.style = modusTableToolbarCss;

exports.modus_pagination = ModusPagination;
exports.modus_select = ModusSelect;
exports.modus_table_filler_column = ModusTableFillerColumn;
exports.modus_table_toolbar = ModusTablePanel;
